...
config_manager = require "trans_svr_config_manager"
io_manager = require "trans_svr_io_manager"

file_reader_manager = require "file_reader_manager"
file_writer_manager = require "file_writer_manager"

trans_svr_upload_manager = require "trans_svr_upload_manager"
trans_svr_manager = require "trans_svr_download_manager"
trans_svr_manager = require "trans_svr_sync_manager"
...
function init_server(vars)
	...
	trans_svr_upload_manager.init(vars)
	if init_ret ~= 0 then
		return init_ret
	end	
	
	trans_svr_download_manager.init(vars)
	if init_ret ~= 0 then
		return init_ret
	end	

	trans_svr_sync_manager.init(vars)
	if init_ret ~= 0 then
		return init_ret
	end	
...
function exit_server(vars)
	...
	trans_svr_upload_manager.exit(vars)
	trans_svr_download_manager.exit(vars)
	trans_svr_sync_manager.exit(vars)	
